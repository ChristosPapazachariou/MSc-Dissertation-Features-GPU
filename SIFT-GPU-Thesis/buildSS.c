...
void g2d(float* target, 
    ...
    norm = 0.0f;
			//logtime("CPU g2d coef1d calc-start");
    for(i = 0; i < gW; i++){
     ...
     ***************************************/
			...
			//logtime("CPU g2d srcPadding-start");
    for(i = 0; i < h+gR*2; i++){
        for(j = 0; j < w+gR*2; j++){
            if( i < gR || i > h+gR-1 
                || j < gR || j > w+gR-1 ){
			...
			//logtime("CPU g2d srcPadding-end");
