...
void buildOB(unsigned char* img, 
    ...
    int i;
			...
			printf("CPUbuildOB call. \n");

    // octave 0 is the original image
			logtime("CPUbuildOB CPU memcpy-start");
    memcpy(octaves[0], img, 
           octavesW[0] * octavesH[0] * 
           sizeof(unsigned char));
			logtime("CPUbuildOB CPU memcpy-end");

    // downsample for octave 1 to O
			logtime("CPU downsample loop -start");
    for(i = 1; i < O; i++){
        downsample(octaves[i], 
			...
			logtime("CPU downsample loop -end");
