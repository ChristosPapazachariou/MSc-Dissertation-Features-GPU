...
void extreme(pointList** keyHead, int* keyN,
    ...
    /********************************************
     * Patch:
     * Eliminate feature points near the border,
    ...
    // gaussian sigmas at each scale
			logtime2("CPU extreme start");
			logtime2("CPU sigma calc start");
    for(i = 0; i < O; i++){
        for(j = 0; j < S; j++){
            sigmas[i][j] = 
                sigma0 * 
			...
			logtime2("CPU sigma calc end");

			logtime2("CPU extrema+edge+keygen start");
    for(i = 0; i < O; i++){
        for(j = 1; j < S-2; j++){
            w = octavesW[i];
            h = octavesH[i];
			...
			logtime2("CPU extrema+edge+keygen end");	
			logtime2("CPU extreme end");
}
